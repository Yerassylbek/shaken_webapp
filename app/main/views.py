...
@main.route("/delete_review/<int:id>")
...
@main.route("/edit_review/<int:id>", methods=['GET', 'POST'])
def edit_review(id):
    review = Review.query.get(id)
    ...
    if current_user == review.user:
        if review:
            print(review)
            form = ReviewForm(formdata=request.form, title=review.review_title, review=review.movie_review)
            if request.method == 'POST' and form.validate():
                review.review_title = form.title.data
                review.movie_review = form.review.data
                review.save_review()
                return redirect(url_for('main.movie', id=review.movie_id))
            return render_template('edit_review.html', title=review.review_title, movie=movie, review_form=form)
        else:
